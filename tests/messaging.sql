...
select public.test_assert(public.test_denied('select public.setup_profile_with_pin(''Different name'',''+12025550123'',''825974'')'),'existing number cannot be overwritten by registration');
select public.test_assert(not (select success from public.recover_profile('+12025550123','825974')),'wrong PIN cannot recover an existing account');
select public.test_assert(not (select profile_ready from public.get_my_profile()),'wrong PIN leaves new device without the existing profile');
reset role;
