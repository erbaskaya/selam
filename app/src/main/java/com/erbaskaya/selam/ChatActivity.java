...
public class ChatActivity extends Activity {
    ...
    private void messageMenu(SupabaseClient.Message m){
        ...
        if(!m.deleted){add(titles,actions,"Yanıtla",()->{replying=m;updateReply();composer.requestFocus();});add(titles,actions,"Kopyala",()->{((android.content.ClipboardManager)getSystemService(CLIPBOARD_SERVICE)).setPrimaryClip(ClipData.newPlainText("Selam mesajı",m.body));toast("Kopyalandı");});
