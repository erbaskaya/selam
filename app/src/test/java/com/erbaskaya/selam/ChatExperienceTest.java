...
public class ChatExperienceTest {
    ...
    @Test @GraphicsMode(GraphicsMode.Mode.NATIVE) public void composerRespectsBothSystemBarsOnSmallScreen(){TestChat a=launch();ViewGroup content=a.findViewById(android.R.id.content);View page=content.getChildAt(0);page.dispatchApplyWindowInsets(new WindowInsets.Builder().setInsets(WindowInsets.Type.systemBars(),android.graphics.Insets.of(0,24,0,48)).setInsetsIgnoringVisibility(WindowInsets.Type.systemBars(),android.graphics.Insets.of(0,24,0,48)).build());page.measure(View.MeasureSpec.makeMeasureSpec(360,View.MeasureSpec.EXACTLY),View.MeasureSpec.makeMeasureSpec(640,View.MeasureSpec.EXACTLY));page.layout(0,0,360,640);assertEquals(24,page.getPaddingTop());assertEquals(48,page.getPaddingBottom());View send=find(page,"Mesajı gönder");assertNotNull(send);Rect bounds=new Rect();send.getDrawingRect(bounds);((ViewGroup)page).offsetDescendantRectToMyCoords(send,bounds);assertTrue("Composer above navigation area",bounds.bottom<=592);assertTrue("Composer is visible",bounds.height()>0);capture(page,"chat");}
    ...
    @Test @GraphicsMode(GraphicsMode.Mode.NATIVE) public void personalizationScreenCanRenderInDarkMode(){Context app=RuntimeEnvironment.getApplication();Appearance look=new Appearance(app);look.put("mode",null,"dark");try(ActivityController<AppearanceActivity> c=Robolectric.buildActivity(AppearanceActivity.class).setup()){assertNotNull(ofType(c.get().getWindow().getDecorView(),ScrollView.class));View page=((ViewGroup)c.get().findViewById(android.R.id.content)).getChildAt(0);page.measure(View.MeasureSpec.makeMeasureSpec(400,View.MeasureSpec.EXACTLY),View.MeasureSpec.makeMeasureSpec(820,View.MeasureSpec.EXACTLY));page.layout(0,0,400,820);capture(page,"personalization-dark");}look.prefs.edit().clear().commit();}
    private void capture(View view,String name){try{java.io.File folder=new java.io.File("build/test-screenshots");folder.mkdirs();android.graphics.Bitmap bitmap=android.graphics.Bitmap.createBitmap(view.getWidth(),view.getHeight(),android.graphics.Bitmap.Config.ARGB_8888);view.draw(new android.graphics.Canvas(bitmap));try(java.io.FileOutputStream out=new java.io.FileOutputStream(new java.io.File(folder,name+".png"))){bitmap.compress(android.graphics.Bitmap.CompressFormat.PNG,100,out);}bitmap.recycle();}catch(Exception e){throw new AssertionError(e);}}
}
